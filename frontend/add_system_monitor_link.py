...
def add_system_monitor_link(file_path):
    """在导航栏中添加系统监控链接"""
    try:
        ...
        # 检查是否已经有系统监控链接
        if '系统监控' in content and 'system-monitor' in content:
            print(f"  跳过（已包含系统监控链接）")
            return False
        
        # 在系统设置之后添加系统监控链接
        pattern = r'(<a href="/system-settings"[^>]*>系统设置</a>)'
        replacement = r'\1\n                    <a href="/system-monitor" class="unified-navbar-menu">系统监控</a>'
...
def main():
    ...
    for filename in files_to_update:
        ...
        if os.path.exists(file_path):
            ...
            if add_system_monitor_link(file_path):
                success_count += 1
            print()
        else:
