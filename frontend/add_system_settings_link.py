...
def add_system_settings_link(file_path):
    """在导航栏中添加系统设置链接"""
    try:
        ...
        # 检查是否已经有系统设置链接
        if '系统设置' in content and 'system-settings' in content:
            print(f"  跳过（已包含系统设置链接）")
            return False
        
        # 在航空器信息之后添加系统设置链接
        pattern = r'(<a href="/aircraft-info"[^>]*>航空器信息</a>)'
        replacement = r'\1\n                    <a href="/system-settings" class="unified-navbar-menu">系统设置</a>'
...
def main():
    ...
    for filename in files_to_update:
        ...
        if os.path.exists(file_path):
            ...
            if add_system_settings_link(file_path):
                success_count += 1
            print()
        else:
